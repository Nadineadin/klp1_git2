...
$stmt = $mysqli->prepare("SELECT gambar FROM fasilitas WHERE id = ? LIMIT 1");
...
// Hapus record
$stmt = $mysqli->prepare("DELETE FROM fasilitas WHERE id = ?");
$stmt->bind_param('i', $id);
if ($stmt->execute()) {
    // hapus file gambar jika ada
    $img_path = __DIR__ . '/../../assets/images/fasilitas/' . ($row['gambar'] ?? '');
